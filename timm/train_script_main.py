...
def _parse_args():
    ...
    if args_config.config:
        config_ext = args_config.config.split('.')[-1]
        if config_ext == "yaml":
            with open(args_config.config, 'r') as f:
                args_ai = yaml.safe_load(f)
    ...
    return args, args_ai, args_text


def training_main(args_ai):
    setup_default_logging()
    args, args_ai_cfg, args_text = _parse_args()
    if args_ai is None:
        args_ai = args_ai_cfg
    ...
    dataset_train = create_dataset(
        args.dataset, root=args.train_data_path, split=args.train_split, is_training=True,
