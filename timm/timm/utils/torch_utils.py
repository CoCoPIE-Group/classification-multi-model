...
def get_mean_and_std(dataset):
    ...
    for inputs, targets in dataloade.r:
        for i in range(3):
...
def model_info(model, input_size=224, verbose=False):
...
def prunned_model_info(model, input_size=224, verbose=False):
    # Plots a line-by-line description of a PyTorch model
    n_p = sum(x.numel() for x in model.parameters())  # number parameters
    n_g = sum(x.numel() for x in model.parameters() if x.requires_grad)  # number gradients
    if verbose:
    ...
    try:  # FLOPS
        # from thop import profile
        macs, n_p = profile(model, inputs=(torch.zeros(1, 3, input_size, input_size),), verbose=False)
        fs = ', %.3f GFLOPS' % (macs / 1E9 * 2)
    except:
        fs = ''
...
def split_train_val(input_folder, output_folder):
    import splitfolders
