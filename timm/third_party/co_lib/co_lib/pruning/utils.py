...
def prune_parse_arguments(parser):
    ...
    admm_args.add_argument('--sp-load-frozen-weights', type=str, help='the weights that are frozen '
...
def get_frozen_weights(model, filename, prune_ratios, frozen_ratios, all_params=False):
    assert (filename)
    ...
    for name in state_dict:
        ...
        if (frozen_ratios is not None and name not in frozen_ratios) or (frozen_ratios is None and name not in prune_ratios):
            continue
...
def build_config_file(model, args):
    """
    current only based on block-punched
    Args:
        model:
        args:
    ...
    """
    prune_ratios = {}
    if args.sparsity_type == 'block_punched':
        named_layers = dict(model.named_modules())
        for i, (name, weights) in enumerate(model.named_parameters()):

            cname = canonical_name(name)
            # clayer_name = '.'.join(cname.split('.')[:-1])
            # if cname == name:
            #     layer_name = clayer_name
            # else:
            #     layer_name  = '.'.join(name.split('.')[:-1])
            # if (layer_name in named_layers and isinstance(named_layers[layer_name],nn.Conv2d)) or \
            #         (clayer_name in named_layers and isinstance(named_layers[clayer_name],nn.Conv2d) ):
            ...
            if shape2d[0] % block_rows != 0 or shape2d[1] % length_x != 0:
                continue
            # print(cname,shape)
            prune_ratios[cname] = args.sp_prune_ratios
    else:
...
def load_configs(model, filename, logger, args):
    if filename is not None:
        try:
            configs = json.loads(filename)
        except ValueError:
        ...
        configs = {}
        configs["prune_ratios"] = build_config_file(model, args)
    args.sp_config_file = json.dumps(configs)
    ...
    if "prune_ratios" in configs:
        ...
        for name in model.state_dict():
            ...
            if cname not in config_prune_ratios:
                continue
            count = count + 1
            if args.sp_prune_ratios and isinstance(args.sp_prune_ratios, float):
                prune_ratios[name] = args.sp_prune_ratios
            else:
                prune_ratios[name] = config_prune_ratios[cname]
            if name != cname:
                logger.info("Map weight config name from {} to {}".format(cname, name))
        ...
        if len(prune_ratios) != len(config_prune_ratios):
            ...
            for name in extra_weights:
                logger.warning("{} in config file cannot be found".format(name))
    ...
    with torch.no_grad():
        for name, W in (model.named_parameters()):
            if name in frozen_weights:
                dtype = W.dtype
                W.data = update_one_frozen_weight(W, frozen_weights[name]['mask'], frozen_weights[name]['weight']).type(dtype)
                pass
        # debug only
        if all_params:
...
def generate_prune_profile(args, model, path, sparsity, override=False):
    if not override:
        print('Not generating profile.')
        return
    ...
    layers_sparsity = {}
    sp_admm_block = eval(args.sp_admm_block)
    for (name, W) in base_model.named_parameters():
        if len(W.shape) == 4:
            layers_sparsity[name] = sparsity

            if W.shape[0] % sp_admm_block[0] != 0 or W.shape[1] % sp_admm_block[1]:
                layers_sparsity[name] = 0

    data = {'prune_ratios': layers_sparsity, 'rho': 0.001}
